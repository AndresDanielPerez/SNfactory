...
class ALPphotonDET:
    ...
    def Zthobs_1sim(self, ma, ga, dNdEdt, Ntargets, exptime, bins, Bth, Nobs, Erange):
        ...
        # Bth: number of EXPECTED background events per bin,
        # Nobs: number of OBSERVED events per bin 
        # Erange: info about the distance SN-Earth, and the energy range of the SRP at Earth during the time window
        """
    ...
    def Zthexp_1sim(self, ma, ga, dNdEdt, Ntargets, exptime, bins, Bth, Erange):
        """ 
        # ma: axion mass,
        # ga: axion coupling,
        # dNdEdt: ALP spectrum generated by a single SN as a function of Ea, ma, ga (internally computes dNgammadEgamma[Ea, ma, ga]),		
	...
	# test statistic
        qmaEXP = 2 * np.sum(Bth[:, 1] * np.log(Bth[:, 1] / (Bth[:, 1] + Sth)) + Sth)
